...
export const getStreaming = ({ queryKey }) => {
  const [, idPart] = queryKey;
  const { id } = idPart;
  return fetch(
    `https://api.themoviedb.org/3/movie/${id}/watch/providers?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
...
export const getSimilarMovies = ({ queryKey }) => {
  ...
  const { id } = idPart;
  return fetch(
    `https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en`
  )
    .then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
